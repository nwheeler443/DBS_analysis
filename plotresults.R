plotresults <- function(comm, path, xaxvalues=c(-10,10), breaks=500) {
	...
	genelist <- genelist <- cbind(dbs[,c(1,2,3,10)], info[match(dbs[,2], info[,1]), 2:ncol(info)])
    ...
    hist(dbs[,10], breaks=breaks, xlim=xaxvalues, main=paste("Results for ", comm, " vs ", path, sep=""), xlab="Delta bitscore")
    library(moments)
