...
png("T3SSgenes.avr.png", width=700, height=500)
par(mar=c(7,5,3,5))
plot(1,0,xlim=c(0,9), ylim=c(0,1500), col="white", xaxt="n", ylab="Bitscore", xlab="", cex.axis=1.4, cex.lab=1.4)
for (i in 1:10) {
	gene <- genes[i]
	points(x=rep(i-1.2, length(pathscores2[!is.na(pathscores2[,gene]),gene])), y=pathscores2[!is.na(pathscores2[,gene]),gene], pch=16, col=rgb(0,0,0,150, max=255))
	points(x=rep(i-1, length(rhizscores2[!is.na(rhizscores2[,gene]),gene])), y=rhizscores2[!is.na(rhizscores2[,gene]),gene], pch=16, col=rgb(0,0,70,150, max=255))
	points(x=rep(i-0.8, length(envscores2[!is.na(envscores2[,gene]),gene])), y=envscores2[!is.na(envscores2[,gene]),gene], pch=16, col=rgb(70,0,0,150, max=255))
}
axis(side=1, at=(0:9), labels=genes[0:10], las=2, cex.axis=1.4)
dev.off()

png("T3SSgenes.hop.png", width=1300, height=500)
par(mar=c(7,5,3,5))
plot(1,0,xlim=c(0,(length(genes)-11)), ylim=c(0,1500), col="white", xaxt="n", ylab="Bitscore", xlab="", cex.axis=1.4, cex.lab=1.4)
for (i in 11:length(genes)) {
	gene <- genes[i]
	points(x=rep(i-11.2, length(pathscores2[!is.na(pathscores2[,gene]),gene])), y=pathscores2[!is.na(pathscores2[,gene]),gene], pch=16, col=rgb(0,0,0,150, max=255))
	points(x=rep(i-11, length(rhizscores2[!is.na(rhizscores2[,gene]),gene])), y=rhizscores2[!is.na(rhizscores2[,gene]),gene], pch=16, col=rgb(0,0,70,150, max=255))
	points(x=rep(i-10.8, length(envscores2[!is.na(envscores2[,gene]),gene])), y=envscores2[!is.na(envscores2[,gene]),gene], pch=16, col=rgb(70,0,0,150, max=255))
}
axis(side=1, at=(0:(length(genes)-11)), labels=genes[11:length(genes)], las=2, cex.axis=1.4)
dev.off()